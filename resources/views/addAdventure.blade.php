...
<div class="relative flex-auto p-4" data-te-modal-body-ref>
    <form action="" method="post" id="adventureForm" enctype="multipart/form-data">
        @csrf
        ...
        <!-- Display uploaded images and allow removal -->
        ...
        <select name="destination_id" id="destination" class="mt-1 p-2 w-full border rounded-md">
            @foreach($destinations as $destination)
                <option class="text-black" value="{{ $destination->id }}">{{ $destination->name }}</option>
            @endforeach
        </select>
        <label for="destination" class="block mt-4 text-sm font-medium text-gray-700">Select Destination</label>
        <select name="destination_id" id="destination" class="mt-1 p-2 w-full border rounded-md">
        ...
        <div class="flex flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
            ...
            <button type="button" class="ml-1 inline-block rounded bg-primary px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-primary-600 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-primary-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-primary-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] dark:shadow-[0_4px_9px_-4px_rgba(59,113,202,0.5)] dark:hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)]" onclick="submitAdventureForm()" data-te-ripple-init data-te-ripple-color="light">
                Save changes
            </button>
        </div>
    </form>
